...
|-----------|------|----------|---------|-------------|
| `baseUrl` | `string \| Ref<string> \| ComputedRef<string>` | ✅ Yes | - | Base URL for all API requests. Can be reactive. |
| `csrf` | `CsrfOptions` | ❌ No | - | CSRF protection configuration |
| `csrf.enabled` | `boolean` | ❌ No | `false` | Enable CSRF token injection |
| `csrf.cookieName` | `string` | ❌ No | `'csrf_token'` | Cookie name containing CSRF token |
| `csrf.headerName` | `string` | ❌ No | `'X-CSRF-Token'` | Header name to send CSRF token |
| `deduplication` | `boolean` | ❌ No | `false` | Enable request deduplication globally (disabled by default) |
| `debug` | `boolean` | ❌ No | `false` | Enable debug logging |
| `autoUpdateDebounceDelay` | `number` | ❌ No | `100` | Debounce time in milliseconds for autoUpdate watches in list/paginated-list actions. Groups rapid parameter changes into a single fetch request. |
| `requestInterceptor` | `RequestInterceptor` | ❌ No | - | Function to modify/abort requests |
| `responseInterceptor` | `ResponseInterceptor` | ❌ No | - | Function to modify/handle responses |
| `fetchOptions` | `RequestInit` | ❌ No | - | Default options for fetch requests |
...
When you need **preconfigured actions** (e.g. a list with fixed filters) but still want the view to override or add filters, ordering, pagination, or options, use the **merge helpers**. They merge default params/options with what the view passes; the view wins on conflicting keys. All merge helper inputs accept `MaybeRefOrGetter` so you can pass refs or computed for reactive defaults (e.g. default filters that depend on current user).

**Import:** `import { mergeListActionArgs, mergeRetrieveActionArgs, mergeMutationActionArgs } from '@fiscozen/data/rest'` (or `from '@fiscozen/data'`).

### mergeListActionArgs (useList / usePaginatedList)

Use when building a custom list or paginated-list action with default filters/options.

**Input:** `defaultParams?`, `defaultOptions?`, `additionalParamsOrOptions?`, `additionalOptions?` (all optional). The first argument from the view can be either params or options; when `additionalOptions` is present, `additionalParamsOrOptions` is treated as params. Otherwise it is discriminated via presence of `filters`, `ordering`, or `pagination`.

**Merge semantics:** Filters and pagination are merged (view wins on same key). Ordering: if the view provides an `ordering` key, use view's; otherwise use default. Options: view wins on same key.

**Filter value semantics (query layer):** `undefined` = omit from request (e.g. remove a default filter in merge); `null` = send to the server (query param present with value null). This applies to both list params and merge helpers.

**Example – custom list with default filters:**

```typescript
// e.g. src/composables/useInvoices.ts
import { useActions, mergeListActionArgs } from '@fiscozen/data'
import type {
  UseListAction,
...
const { useList: useInvoicesListBase } = useActions<Invoice>('invoices/')

/**
 * List composable with default STS filters; view can override filters, ordering, pagination, options.
 * Same overloads as useList: (), (paramsOrOptions), (params, options).
 * (Params/options inherit T from UseListAction<Invoice>; omit generic only when you don't need initialData.)
 */
export const useInvoicesList: UseListAction<Invoice> = (
  paramsOrOptions?: UseListActionParamsOrOptions<Invoice>,
  options?: UseListActionOptions<Invoice>
) => {
  ...
  return useInvoicesListBase(mergedParams, mergedOptions)
}
```

**Example – custom paginated list with default filters:**

```typescript
// same file or another composable module
import { useActions, mergeListActionArgs } from '@fiscozen/data'
import type {
...
const { usePaginatedList: useInvoicesPaginatedListBase } = useActions<Invoice>('invoices/')

/**
 * Paginated list composable with default filters and options; view can override.
 * Same overloads as usePaginatedList: (), (paramsOrOptions), (params, options).
 * (Params/options inherit T from UsePaginatedListAction<Invoice>; omit generic only when you don't need initialData.)
 */
export const useInvoicesPaginatedList: UsePaginatedListAction<Invoice> = (
  paramsOrOptions?: UsePaginatedListParamsOrOptions,
  options?: UsePaginatedListActionOptions
) => {
  const { params: mergedParams, options: mergedOptions } = mergeListActionArgs({
    defaultParams: {
  ...
  return useInvoicesPaginatedListBase(mergedParams, mergedOptions)
}
```

**In the view:** Same API as `useList` / `usePaginatedList`; you can override or add filters, ordering, pagination, options.

```vue
<script setup>
import { useInvoicesList, useInvoicesPaginatedList } from '@/composables/useInvoices'

// List – override ordering and pagination
...
// PaginatedList – override page only; default filters and pageSize apply
...
Use when building a custom retrieve action with default pk or options. At least one of `defaultPk` or `overridePk` should be provided. When provided, `overridePk` overrides the default (substitute pk). The view's `overridePk` is preserved as-is (refs kept for reactivity).

**Input:** `defaultPk?`, `defaultOptions?`, `overridePk?`, `additionalOptions?`.

### mergeMutationActionArgs (useCreate / useUpdate / useDelete)

Use when building a custom mutation action with default options (e.g. `throwOnError: true`).

**Input:** `defaultOptions?`, `additionalOptions?`.
...
- **Nested resources** – Endpoints like `users/68/stats` where `stats` is a nested resource under a user
- **Custom endpoints** – Non-CRUD endpoints like exports, searches, or bulk operations
- **Complex query strings** – Endpoints requiring custom query parameters that don't fit the standard filters/ordering/pagination pattern
- **Non-standard URL patterns** – Endpoints that don't follow REST conventions


### Example 1: Nested resource – user stats
...
### Example 2: Custom endpoint with query string – export invoices
...
|-----------|------|----------|-------------|
| `pk` | `string \| number \| Ref \| ComputedRef` | ✅ Yes | Primary key of the entity. Can be reactive - changes trigger refetch. |
| `options` | `UseRetrieveActionOptions<T>` | ❌ No | Query options (see below) |
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `params` | `UseListActionParams` | ❌ No | Query parameters (see below) |
| `options` | `UseListActionOptions<T>` | ❌ No | Query options (see below) |
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `filters` | `Record<string, string \| number \| boolean \| null \| undefined> \| Ref \| ComputedRef` | ❌ No | Initial filter parameters. **Query semantics:** `undefined` = omit from request; `null` = send to server. After initialization, modify the returned `filters` reactive object directly. |
| `ordering` | `Array<Record<string, 'asc' \| 'desc' \| 'none'>> \| Ref \| ComputedRef` | ❌ No | Initial ordering parameters as array (e.g., `[{ name: 'asc' }, { created_at: 'desc' }]`). Normalized to query string format: `ordering=name,-created_at`. After initialization, modify the returned `ordering` reactive array directly. |
| `pagination` | `{ page?: number, pageSize?: number } \| Ref \| ComputedRef` | ❌ No | Initial pagination parameters. If provided (even if empty), default values are applied: `page` defaults to `1`, `pageSize` defaults to `50`. After initialization, modify the returned `pagination` reactive object directly. |
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `params` | `UsePaginatedListActionParams` | ❌ No | Same as `UseListActionParams` (filters, ordering, pagination) |
| `options` | `UsePaginatedListActionOptions<T>` | ❌ No | Options including `dataKey` and `enableSingleOrdering` |
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `options` | `UseCreateActionOptions` | ❌ No | Mutation options (see below) |
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `options` | `UseUpdateActionOptions` | ❌ No | Mutation options (see below) |
...
| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `partialUpdate` | `boolean` | ❌ No | `true` | `true` = PATCH (partial), `false` = PUT (full replacement) |
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `options` | `UseDeleteActionOptions` | ❌ No | Mutation options (see below) |
...
| Option | Type | Required | Default | Description |
|--------|------|----------|---------|-------------|
| `onMount` | `boolean` | ❌ No | `true` | Automatically execute on component mount |
| `autoUpdate` | `boolean \| Ref<boolean> \| ComputedRef<boolean>` | ❌ No | `true` | Automatically refetch when reactive parameters change |
| `initialData` | `T \| null` | ❌ No | `null` | Initial data before request completes |
| `deduplication` | `boolean` | ❌ No | `undefined` | Enable request deduplication (overrides global setting) |
| `throwOnError` | `boolean` | ❌ No | `false` | Whether to throw errors instead of storing them in the error ref |
...
- **Merge Helpers for Custom Actions** - `mergeListActionArgs`, `mergeRetrieveActionArgs`, `mergeMutationActionArgs` to build preconfigured actions (default params/options) while letting the view override or add filters, ordering, pagination, options
