...
6. [Custom Actions and Nested Routes](#custom-actions-and-nested-routes)
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `filters` | `Record<string, string \| number \| boolean \| null \| undefined> \| Ref \| ComputedRef` | ❌ No | Initial filter parameters. `null`/`undefined` values are excluded. After initialization, modify the returned `filters` reactive object directly. |
...
## Custom Actions and Nested Routes
...
**Scenario:** Fetch user statistics from endpoint `GET /users/{userId}/stats`
...
export const useUsers = () => {
  ...
  /**
   * Fetch user statistics from nested endpoint
   * 
   * @param userId - User ID (can be reactive ref or computed)
   * @returns useFzFetch result with data, error, isLoading, execute
   * 
   * @example
   * const userId = ref(68)
  ...
  const useGetUserStats = (userId: MaybeRefOrGetter<number>) => {
    // Create reactive URL that updates when userId changes
    const url = computed(() => `users/${toValue(userId)}/stats`)
    
    // useFzFetch handles reactive URLs automatically
...
// Reactive user ID - stats auto-refetch when userId changes
const userId = ref(68)
const { data: stats, error, isLoading } = useGetUserStats(userId)

// Change user ID to load different stats
const loadStatsForUser = (id: number) => {
  userId.value = id // Automatically refetches stats
}
</script>

<template>
  <!-- Template: Display stats data, handle loading/error states -->
  <!-- Focus on data layer logic, not UI implementation -->
</template>
```

**Key Points:**

- Pass `userId` as parameter when creating the function: `useGetUserStats(userId)`
- URL is reactive - automatically refetches when `userId` changes
- Returns standard `useFzFetch` interface: `{ data, error, isLoading, execute }`
- Works with `ref`, `computed`, or static values

### Example 2: Custom Endpoint with Query String - Export Invoices

**Scenario:** Export invoices as ZIP file from endpoint `GET /esporta/lista_fatture/{invoiceIds}/?zip`
...
export const useInvoices = () => {
  ...
  /**
   * Export invoices as file (ZIP or PDF)
   * 
   * @param invoiceIds - Array of invoice IDs to export (can be reactive)
   * @param format - Export format: 'zip' or 'pdf' (default: 'zip')
   * @returns useFzFetch result with Blob data, error, isLoading, execute
   * 
   * @example
   * const invoiceIds = ref([1, 2, 3])
   * const { data: file, execute: exportInvoices } = useExportInvoices(invoiceIds, 'zip')
   * await exportInvoices() // Triggers download
   */
  const useExportInvoices = (
    invoiceIds: MaybeRefOrGetter<number[]>,
    format: 'zip' | 'pdf' = 'zip'
  ) => {
    // Create reactive URL with invoice IDs
    const url = computed(() => {
      ...
      // Build URL: esporta/lista_fatture/1-2-3
      return `esporta/lista_fatture/${ids.join('-')}`
    })
    
    // useFzFetch with query parameters for format
    // Adds ?zip=true or ?pdf=true to the URL
...
// Selected invoice IDs (can be from checkbox selection)
const selectedInvoiceIds = ref<number[]>([1, 2, 3, 5])
const exportFormat = ref<'zip' | 'pdf'>('zip')

// Create export function with reactive parameters
const { 
  data: fileBlob, 
  error, 
  isLoading: isExporting, 
  execute: exportInvoices 
} = useExportInvoices(selectedInvoiceIds, exportFormat)

/**
 * Trigger export and download file
 */
const handleExport = async () => {
  ...
  try {
    ...
    // Check for errors
    if (error.value) {
      console.error('Export failed:', error.value.message)
      return
    }
    
    // Download the blob file
    // Handle fileBlob.value download (create download link, trigger download, cleanup)
    if (fileBlob.value) {
 ...
 */
const changeFormat = (format: 'zip' | 'pdf') => {
  exportFormat.value = format
  // URL automatically updates with new query parameter
  // Call execute() again to fetch with new format
}
</script>

<template>
  <!-- Template: Invoice selection, format selection, export button, error display -->
  <!-- Focus on data layer logic, not UI implementation -->
</template>
```
...
- Pass parameters when creating the function: `useExportInvoices(invoiceIds, format)`
- Query parameters are added via `queryParams` option in `useFzFetch`
- URL is reactive - updates when `invoiceIds` or `format` change
- Manual execution required - call `execute()` to trigger the request
