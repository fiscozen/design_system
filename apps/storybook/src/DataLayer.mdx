...
3. [Setup](#setup) (includes Interceptors, Deduplication, Default Headers)
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>No</td>
      <td>-</td>
      <td>Normalize path trailing slash: <code>true</code> = always add, <code>false</code> = always remove, <code>null</code>/omit = don't change. Per-action override: pass <code>trailingSlash</code> in action options (useRetrieve, useList, useCreate, useUpdate, useDelete) to override setup for that action; when using <code>useFzFetch</code> directly, pass <code>trailingSlash</code> in UseFzFetchOptions. <code>null</code> = no normalization for that request, omit = use setup.</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>No</td>
      <td>-</td>
      <td>Default options for every fetch request (credentials, mode, headers, etc.). Headers defined here are merged with per-request headers; per-request values override defaults. Set a header to <code>undefined</code> per-request to remove it.</td>
    </tr>
  </tbody>
</table>
...
Headers configured in `fetchOptions.headers` are automatically included in every request and merged with per-request headers.
...
**Merging behavior:** Per-request headers are merged with setup headers. When both define the same key, the per-request value wins.

```typescript
// Setup defines X-Realm: 'frontoffice'
const { execute } = useFzFetch<User>('/users', {
  method: 'POST',
  body: null,
  headers: {
    'Content-Type': 'application/json',  // added
    'X-Realm': 'backoffice'              // overrides setup default
  }
}, { immediate: false })
// Final headers: X-Realm: 'backoffice', X-App: 'web', Content-Type: 'application/json'
```

**Removing a default header:** Set the header value to `undefined` in per-request headers to exclude it from that request.

```typescript
// Setup defines X-Realm: 'frontoffice' and X-App: 'web'
const { execute } = useFzFetch<User>('/users', {
  headers: { 'X-Realm': undefined }   // removes X-Realm for this request only
})
// Final headers: X-App: 'web' (X-Realm excluded)
```

This also works with reactive headers:
...
// First call: X-Realm removed
await execute()

// Change headers: X-Realm restored, Content-Type added
headers.value = { 'Content-Type': 'application/json' }
await execute()
// Final headers: X-Realm: 'frontoffice', X-App: 'web', Content-Type: 'application/json'
```
...
**Query options (useRetrieve, useList, usePaginatedList):** `onMount` (default true), `autoUpdate` (default true), `initialData`, `deduplication`, `throwOnError` (default false), `trailingSlash` (true/false/null; overrides setup for that action). **Mutation options:** `throwOnError` (default false), `trailingSlash` (true/false/null; overrides setup for that action).
...
**Options:** `immediate` (default true), `refetch`, `initialData`, `deduplication`, `trailingSlash` (true/false/null; null = no normalization for this request, omit = use setup).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><code>true | false | null</code></td>
      <td>No</td>
      <td><code>undefined</code> (use setup)</td>
      <td>Override setup trailing slash for this action: <code>true</code> = add trailing slash, <code>false</code> = remove, <code>null</code> = no normalization</td>
    </tr>
  </tbody>
</table>
...
const { data, execute } = useListUsers(
  { filters: { enabled: true } },
  {
    ...
    trailingSlash: false   // Override: no trailing slash for this list (e.g. API expects /users)
  }
)
```
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>No</td>
      <td><code>undefined</code> (use setup)</td>
      <td>Override setup trailing slash for this action: <code>true</code> = add, <code>false</code> = remove, <code>null</code> = no normalization</td>
    </tr>
  </tbody>
</table>
...
const { execute: createCreditNote } = useCreateCreditNote({
  ...
  trailingSlash: false  // Override: e.g. API expects POST /invoices without trailing slash
})
```

**Per-action trailing slash override**

You can override the global `trailingSlash` setup for a single action by passing `trailingSlash` in that action's options. Useful when one endpoint expects a different convention than the rest.

```typescript
// Setup: global trailing slash = true (e.g. /users/, /users/1/)
setupFzFetcher({ baseUrl: 'https://api.example.com', trailingSlash: true })

const { useRetrieve, useList, useCreate } = useActions<User>('users')

// This list request uses /users/ (setup default)
const { data: users } = useListUsers()

// Override: this retrieve uses /users/1 (no trailing slash) for a legacy endpoint
const { data: user } = useRetrieveUserById(1, { trailingSlash: false })

// Override: this create uses POST /users (no trailing slash)
const { execute: createUser } = useCreateUser({ trailingSlash: false })
```
