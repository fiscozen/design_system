...
```typescript
import { useActions, mergeListActionArgs } from '@fiscozen/data/rest'
import type { ListActionParams, QueryActionOptions } from '@fiscozen/data/rest'

interface Invoice { id: number; name: string }

export const useSTS = () => {
  const { useList: useInvoicesList } = useActions<Invoice>('invoices/')

  const useSTSLists = (
    paramsOrOptions?: ListActionParams | QueryActionOptions<Invoice[]>,
    options?: QueryActionOptions<Invoice[]>
  ) => {
...
**In the view:** Same API as `useList`; you can override or add filters, ordering, pagination, options.

```vue
<script setup>
const { useSTSLists } = useSTS()
const { data, filters, pagination, ordering } = useSTSLists(
...
| `options` | `QueryActionOptions<T>` | ❌ No | Query options (see below) |
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `params` | `ListActionParams` | ❌ No | Query parameters (see below) |
| `options` | `QueryActionOptions<T[]>` | ❌ No | Query options (see below) |
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `options` | `MutationActionOptions` | ❌ No | Mutation options (see below) |
