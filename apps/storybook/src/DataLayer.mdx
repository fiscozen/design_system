...
import { useActions, callListActionWithDefaults } from '@fiscozen/data'
...
export const useInvoicesList = (paramsOrOptions?, options?) =>
  callListActionWithDefaults(useInvoicesListBase, {
    defaultParams: {
...
```typescript
import { useActions, callPaginatedListActionWithDefaults } from '@fiscozen/data'

const { usePaginatedList: useInvoicesPaginatedListBase } = useActions<Invoice>('invoices/')

export const useInvoicesPaginatedList = (paramsOrOptions?, options?) =>
  callPaginatedListActionWithDefaults(useInvoicesPaginatedListBase, {
    defaultParams: {
...
**Example – custom retrieve with default pk:**

```typescript
import { useActions, callRetrieveActionWithDefaults } from '@fiscozen/data'

const { useRetrieve: useRetrieveInvoiceBase } = useActions<Invoice>('invoices/')

export const useRetrieveInvoice = (pk?, options?) =>
  callRetrieveActionWithDefaults(useRetrieveInvoiceBase, {
    defaultPk: userId, // ref/getter supported
    overridePk: pk,
    additionalOptions: options,
  })
```

**Example – custom create with default options:**

```typescript
import { useActions, callCreateActionWithDefaults } from '@fiscozen/data'

const { useCreate: useCreateInvoiceBase } = useActions<Invoice>('invoices/')

export const useCreateInvoice = (options?) =>
  callCreateActionWithDefaults(useCreateInvoiceBase, {
    defaultOptions: { throwOnError: true },
    additionalOptions: options,
  })
```

**In the view:** Same API as the base actions; you can override or add params/options.
