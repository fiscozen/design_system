...
When you need **preconfigured actions** (e.g. a list with fixed filters) but still want the view to override or add filters, ordering, pagination, or options, use the **merge helpers**. They merge default params/options with what the view passes; the view wins on conflicting keys.
...
**Filter values:** Use `undefined` to remove a default filter (omit from request). Use `null` to send `null` to the server.
