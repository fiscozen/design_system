...
> ⚠️ **Deprecated.** Use [`FzSelect`](./?path=/docs/documentation-form-fzselect--docs) from `@fiscozen/select` with `filterable: true` (and optionally `filterFn`) instead. The package and its npm releases stay in place for backward compatibility, but no new features will be developed. See [Migration to FzSelect](#migration-to-fzselect) below.
...
`FzSelect` is a superset of `FzTypeahead`: same floating panel, lazy loading, Fuse.js-powered filtering, and keyboard navigation, exposed through a corrected prop name and a wider option-value type.
...
<table>
  ...
  <tbody>
    <tr>
      <td><code>filtrable</code> (default <code>true</code>)</td>
      <td><code>filterable</code> (default <code>false</code>)</td>
      <td>Spelling fix <strong>and</strong> default flipped. To keep typeahead behavior you must explicitly pass <code>:filterable="true"</code>.</td>
    </tr>
    <tr>
      <td><code>filterFn</code></td>
      <td><code>filterFn</code></td>
      <td>Same signature. Available only when <code>filterable</code> is <code>true</code>.</td>
    </tr>
    <tr>
      <td><code>delayTime</code>, <code>fuzzySearch</code></td>
      <td><code>delayTime</code>, <code>fuzzySearch</code></td>
      <td>Identical defaults (500ms / true).</td>
    </tr>
    <tr>
      <td><code>options: FzTypeaheadOptionsProps[]</code></td>
      <td><code>options: FzSelectOptionsProps[]</code></td>
      <td>Same shape; option <code>value</code> is now <code>string | number</code> (was <code>string</code> only).</td>
    </tr>
    <tr>
      <td><code>fztypeahead:select</code> (payload <code>value</code>)</td>
      <td><code>fzselect:select</code> (payload <code>value, option</code>)</td>
      <td>Now also receives the full option object as the second argument.</td>
    </tr>
    <tr>
      <td>—</td>
      <td><code>fzselect:clear</code></td>
      <td>New explicit clear event.</td>
    </tr>
    <tr>
      <td><code>update:modelValue</code> (<code>string | undefined</code>)</td>
      <td><code>update:modelValue</code> (<code>string | number | undefined</code>)</td>
      <td>Wider type to match the option <code>value</code>.</td>
    </tr>
    <tr>
      <td>All other props (<code>label</code>, <code>placeholder</code>, <code>required</code>, <code>disabled</code>, <code>readonly</code>, <code>error</code>, <code>clearable</code>, <code>leftIcon</code>/<code>rightIcon</code>, <code>rightIconButton</code>, <code>variant</code>, <code>environment</code>, <code>optionsToShow</code>, <code>floatingPanelMaxHeight</code>, <code>position</code>, <code>teleport</code>, …)</td>
      <td>Same names, same defaults</td>
      <td>No changes required.</td>
    </tr>
  </tbody>
</table>

`FzSelect` also adds props absent from `FzTypeahead`: `highlighted` / `highlightedDescription` (warning state, supports `v-model:highlighted`) and `aiReasoning` / `aiReasoningDescription` (AI-suggestion state, supports `v-model:aiReasoning`).

### Understanding `filterable` + `filterFn`

- Setting `filterable: true` makes the opener swap to an input when the dropdown is open; typing then filters the list.
- When you also pass `filterFn`, `FzSelect` calls it with the debounced search text (debounced by `delayTime`, default 500ms) and uses its return value as the options list, bypassing the built-in Fuse.js / substring matcher entirely.
...
**Filterable typeahead with custom (remote) filter:**

```vue
<!-- Before: @fiscozen/typeahead -->
<FzTypeahead
  v-model="selected"
  :options="options"
...
**Typeahead used as a plain select (`filtrable: false`):**

```vue
<!-- Before -->
<FzTypeahead v-model="selected" :options="options" :filtrable="false" />

<!-- After: drop the prop entirely — FzSelect is non-filterable by default -->
<FzSelect v-model="selected" :options="options" />
```
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><code>undefined</code></td>
      <td>Custom filter function (can be async). When provided, overrides the default Fuse.js filtering. <strong>Only available when <code>filtrable: true</code></strong></td>
    </tr>
    <tr>
      <td><code>delayTime</code></td>
      <td><code>number</code></td>
      <td><code>500</code></td>
      <td>Delay in milliseconds before applying filter after user stops typing (debounce time). <strong>Only available when <code>filtrable: true</code></strong></td>
    </tr>
    <tr>
      <td><code>fuzzySearch</code></td>
      <td><code>boolean</code></td>
      <td><code>true</code></td>
      <td>If true, uses Fuse.js for fuzzy search (handles typos and partial matches). If false, uses simple case-insensitive substring matching. <strong>Only available when <code>filtrable: true</code></strong></td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td><code>boolean</code></td>
      <td><code>false</code></td>
      <td>If true, right icon is rendered as an interactive button instead of a static icon. Only shown when filtrable is false (button mode)</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>Visual variant of the typeahead. Only applicable when filtrable is false</td>
    </tr>
    <tr>
...
<template>
  <FzTypeahead
    ...
    @fztypeahead:right-icon-click="handleRightIconClick"
  />
</template>
```
...
> ⚠️ `FzTypeahead` is deprecated. For **all new code**, use `FzSelect` from `@fiscozen/select` with `filterable: true` (and optionally `filterFn`). The guidance below describes scenarios where `FzTypeahead` was historically used — apply it to `FzSelect` going forward.
...
- **New code**: Do not introduce new `FzTypeahead` usages; use `FzSelect` instead.
