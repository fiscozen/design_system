...
export const PreviewAllIcons: Story = {
  render: () => ({
    components: { FzIconBackground, FzButton },
    setup() {
    ...
    methods: {
      copyCode(icon: string, variant: string) {
        ...
        const value = `<FzIconBackground name="${icon}" variant="${variant}"/>`
        el.value = value
    ...
    template: `
      <div>
        <h1 class='font-bold px-10 text-3xl mb-10'>Icon Background List</h1>
        ...
        <div v-for="prefix in Object.keys(mappedIconsByPrefix)" :key="prefix" class="flex flex-col p-10 gap-10 mb-10">
          ...
          <div class='flex flex-row flex-wrap gap-20 overflow-hidden mt-20'>
            <div v-for="icon in mappedIconsByPrefix[prefix]"
              ...
              <FzIconBackground :name="icon" :size="size" :variant="prefix" />
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Verify icon list header is present', async () => {
      const header = canvas.getByText('Icon Background List')
...
export const BackgroundColorCustom: Story = {
  args: {
    name: 'bell',
    size: 'lg',
  ...
  play: async ({ canvasElement, step }) => {
    await step('Verify icon container has custom background class', async () => {
      const iconContainer = canvasElement.querySelector('span.flex')
      await expect(iconContainer).toHaveClass('bg-grey-100')
    })
    await step('Verify background wrapper classes are present', async () => {
      const iconContainer = canvasElement.querySelector('span.flex')
      await expect(iconContainer).toHaveClass('box-content')
      await expect(iconContainer).toHaveClass('rounded-full')
    })
  }
}
