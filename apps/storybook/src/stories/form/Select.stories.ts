  ...
  play: async ({ args, canvasElement, step }) => {
    ...
    await step('Open dropdown and verify selected option is highlighted', async () => {
      ...
      const opener = buttons.find(
        (btn) => btn.getAttribute('test-id') === 'fzselect-opener'
      ) as HTMLElement
      await userEvent.click(opener)
  ...
  play: async ({ args, canvasElement, step }) => {
    ...
    await step('Select an option and verify update:modelValue IS called', async () => {
      ...
      if (optionOne) {
        ...
        const openerAfter = buttonsAfter.find(
          (btn) => btn.getAttribute('test-id') === 'fzselect-opener'
        )
        await expect(openerAfter?.textContent).toContain('One')
      }
    ...
    await step('Verify clicking selected option does not clear (clearable=false)', async () => {
      ...
      if (selectedOption) {
        ...
        // Verify value is still selected (not cleared)
        const buttonsAfter = canvas.getAllByRole('button')
        const openerStillSelected = buttonsAfter.find(
          (btn) => btn.getAttribute('test-id') === 'fzselect-opener'
        )
    ...
    await step('Verify can select different option and update:modelValue IS called', async () => {
      ...
      if (optionTwo) {
        ...
        const lastCall =
          args['onUpdate:modelValue'].mock.calls[args['onUpdate:modelValue'].mock.calls.length - 1]
...
export const Highlighted: SelectStory = {
  ...Template,
  args: {
    ...Template.args,
    highlighted: true,
  ...
  play: async ({ args, canvasElement }) => {
    ...
    await userEvent.click(opener!)
    await waitFor(() => {
      const option = document.querySelector('button[role="option"]')
      expect(option).toBeTruthy()
    })
    const option = document.querySelector('button[role="option"]') as HTMLElement
    await userEvent.click(option)
    await waitFor(() => {
      expect(args['onUpdate:highlighted']).toHaveBeenCalledWith(false)
    })
  }
}

export const HighlightedBackoffice: SelectStory = {
  ...Template,
  args: {
    ...Template.args,
    highlighted: true,
...
export const AIReasoning: SelectStory = {
  ...Template,
  args: {
    ...Template.args,
    aiReasoning: true
...
export const AIReasoningWithLeftIcon: SelectStory = {
  ...Template,
  args: {
    ...Template.args,
    aiReasoning: true,
  ...
  play: async ({ canvasElement }) => {
    // Sparkles should NOT be rendered when leftIcon is provided
    const sparklesIcon = canvasElement.querySelector('.fa-sparkles')
    await expect(sparklesIcon).toBeNull()

    // Left icon should be rendered instead
    const bellIcon = canvasElement.querySelector('.fa-bell')
    await expect(bellIcon).toBeInTheDocument()

    // Left icon should be hidden from assistive technology
    const bellIconWrapper = bellIcon?.closest('[aria-hidden="true"]')
    await expect(bellIconWrapper).toBeInTheDocument()
  }
}

export const HighlightedError: SelectStory = {
  ...Template,
  args: {
    ...Template.args,
    highlighted: true,
  ...
  play: async ({ canvasElement }) => {
    ...
    // Error should take priority — verify error styling is shown
    const buttons = canvas.getAllByRole('button')
    const opener = buttons.find((btn) => btn.getAttribute('test-id') === 'fzselect-opener')
    await expect(opener).toHaveAttribute('aria-invalid', 'true')
  }
...
export const HighlightedDisabled: SelectStory = {
  ...Template,
  args: {
    ...Template.args,
    highlighted: true,
...
export const AIReasoningFilterable: SelectStory = {
  ...Template,
  args: {
    ...Template.args,
    aiReasoning: true,
  ...
  play: async ({ canvasElement }) => {
    ...
    // Verify sparkles icon on the button (before opening)
    const sparklesIcon = canvasElement.querySelector('.fa-sparkles')
    await expect(sparklesIcon).toBeInTheDocument()
