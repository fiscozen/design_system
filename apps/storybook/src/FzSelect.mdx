...
- **Filterable Mode**: Enable with `filtrable: true` - options are filtered as the user types using Fuse.js or custom filter functions
...
To enable filtering, set `filtrable` to `true`:
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Visual variant of the select. Only applicable when filtrable is false</td>
    </tr>
    <tr>
...
- **Small Fixed Lists**: For small, fixed lists (less than 10 items), use `filtrable: false` to disable filtering
