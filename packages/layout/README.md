...
`<main>`. `FzFocusTemplate` and `FzListTemplate` have a non-landmark container
`<div>` as their root, so fall-through attributes land on that container,
**not** on the inner `<main>` — label the content directly if a `main`
