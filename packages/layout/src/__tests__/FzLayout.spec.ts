...
describe('FzLayout', () => {
  ...
  describe('Props', () => {
    ...
    describe('disablePadding prop', () => {
      ...
      it('should omit p-12 from regions when disablePadding is true', async () => {
        ...
        await wrapper.vm.$nextTick()
        expect(wrapper.find('.fz-layout__header').classes()).not.toContain('p-12')
        expect(wrapper.find('.fz-layout__main').classes()).not.toContain('p-12')
  ...
  describe('Edge Cases', () => {
    ...
    it('should track the viewport via matchMedia, not a window resize handler', async () => {
      ...
      expect(addSpy.mock.calls.some(([event]) => event === 'resize')).toBe(false)
    ...
    it('should ignore breakpoint tokens outside its own scale (e.g. `desktop`)', async () => {
      ...
      expect(wrapper.classes().some((cls) => cls === 'fz-layout__oneColumn--desktop')).toBe(false)
    })
