...
const activePage = defineModel<number>("activePage", { default: 1 });
...
const prefixedPaginationUrlKey = computed(() =>
  props.urlKeyPrefix
    ? `${props.urlKeyPrefix}_${props.paginationUrlKey}`
    : props.paginationUrlKey,
);
...
<template>
  ...
  >
    ...
    <div
      class="fz__table__footer mt-[20px]"
      v-if="pages && pages > 1 && internalValue?.length"
    >
      <FzPagination
        :totalPages="pages"
        v-model:currentPage="activePage"
        :syncUrl="props.paginationSyncUrl"
        :urlKey="prefixedPaginationUrlKey"
