...
const filters = computed({
  get() {
    const res = columns.value
      .filter((col) => col.props.filterable)
      .reduce(
        (acc, column) => {
          acc[column.props.field || column.props.header] =
            column.props.filterName || column.props.header;
          return acc;
        },
...
const getHeaderClasses = (column: FzColumnProps) => {
  return {
    ...
    "justify-end":
      column.headerTextJustify === "right" ||
      typeof column.numeric !== "undefined",
  };
};
...
const internalValue = computed(() => {
  ...
  const safeOrdering = Object.entries(ordering.value)
    .filter(
      ([key, val]) =>
        !!val.orderable && !!val.direction && val.direction !== "none",
    )
    .map(([key, val]) => {
...
const openRowIds = defineModel<Set<string | number>>("openRowIds", {
  default: () => new Set(),
});
...
const indeterminate = computed(() => {
  return (
    selectedRowIds.value && selectedRowIds.value.size > 0 && !allSelected.value
  );
});
...
<template>
  <div
    ref="tableContainer"
    :class="[
      'fz-table-container m-0 p-0 size-full text-grey-500 relative flex flex-col',
      tableClass,
    ]"
  >
    ...
    >
      ...
      <FzButton
        ...
        :iconAndLabel="!!newItemButtonIcon"
        :iconName="newItemButtonIcon"
        size="lg"
    ...
    <div ref="scrollContainer" class="fz__table overflow-auto relative">
      ...
      <div
        :class="[
          staticClasses,
          { 'min-h-[200px]': !(internalValue?.length || rows?.length) },
        ]"
        :style="{
      ...
      >
        <template
          v-if="!(variant == 'list' || (variant === 'radio' && smOrSmaller))"
        >
          <div
            v-if="variant === 'accordion'"
            :class="['fz__table__header--accordion', headerStaticClasses]"
          ></div>
          <div
            v-if="variant === 'radio'"
            :class="['fz__table__header--radio', headerStaticClasses]"
          ></div>
          <div
            v-if="selectable"
            :class="[
          ...
          <div
            ...
            role="columnheader"
            :aria-sort="getAriaSort(column.props)"
          >
            {{ column.props.header }}
            <FzIcon
              ...
              :class="{
                'text-blue-500':
                  getOrdering(column.props)?.direction !== 'none',
              }"
              data-cy="fztable-ordering"
              :name="orderingIconName(column.props)"
              size="md"
              variant="fas"
        ...
        <template
          v-if="
            ...
            ['normal', 'radio', 'list'].includes(variant)
          "
        >
          <slot v-for="(row, index) in internalValue" :name="`row-${index}`">
            <FzRow
              ...
              :columns="columns"
              :data="row"
              :isList="['radio', 'list'].includes(variant)"
              :isOverflowing
              :actions="
              ...
              "
              @fztable:rowactionclick="
                (...args) => emit('fztable:rowactionclick', ...args)
              "
              :selectable="props.selectable"
              :hasRadio="props.variant === 'radio'"
        ...
        >
          <FzRow
            ...
            :showLeftCol="true"
            :leftColIconClass="
              row.subRows?.length && openRowIds.has(row.id || index)
                ? 'text-blue-500'
                : ''
            "
            :leftColIcon="
              row.subRows?.length
                ? openRowIds.has(row.id || index)
                  ? 'angle-up'
                  : 'angle-right'
                : undefined
            "
          ...
          <template
            ...
            v-if="openRowIds.has(row.id || index)"
            :key="subindex"
          >
            <FzRow
              :id="subindex"
        ...
        <div
          v-else
          class="fz__table__empty h-full self-center justify-self-center min-h-[200px] flex justify-center items-center col-span-full"
        >
    ...
    <div
      class="fz__table__footer w-full overflow-x-auto mt-[20px]"
      v-if="pages && pages > 1 && internalValue?.length"
    >
      <div
        class="fz__table__pagination flex flex-row justify-end items-center gap-4 min-w-fit ml-auto"
      >
        <FzButton
          v-if="!smOrSmaller"
          @click="activePage--"
        ...
        >
        <div
          class="fz__pagination__separator size-32 flex justify-center items-center"
          v-if="activePage - effectivePageInterval - 1 > 0"
        >
          <span>...</span>
        </div>
        <FzButton
        ...
        <div
          class="fz__pagination__separator size-32 flex justify-center items-center"
          v-if="pages - activePage - effectivePageInterval - 2 > 0"
        >
          <span>...</span>
        </div>
        <FzButton
  ...
  <Transition name="filters" v-if="!smOrSmaller">
    ...
    >
      ...
      <div class="fztable__filters__container grow flex flex-col overflow-auto">
        <div
          class="flex flex-col w-full mt-32 px-12"
      ...
      <div
        class="flex flex-row items-center h-64 p-12 border-t-1 border-solid border-gray-100 justify-self-end"
      >
        <FzButton
          variant="danger"
  ...
  >
    <template #body>
      <div
        class="flex flex-col w-full mt-32 px-12 grow overflow-auto"
        v-for="(filter, filterKey) in filters"
        :key="filterKey"
      >
    ...
    <template #footer>
      <div class="flex flex-row w-full justify-between">
        <FzButton
          ...
          >Cancella filtri</FzButton
        >
        <FzButton variant="primary" @click="() => filtersDialog.close()"
          >Chiudi</FzButton
        >
      </div>
    </template>
