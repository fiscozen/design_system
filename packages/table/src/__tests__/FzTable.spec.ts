...
describe("FzTable", () => {
  ...
  describe("Props", () => {
    describe("variant prop", () => {
      it("should render normal variant without accordion or radio header cells", async () => {
        const wrapper = createWrapper({ variant: "normal" });
        await wrapper.vm.$nextTick();
        expect(wrapper.find(".fz__table__header--accordion").exists()).toBe(
          false,
        );
        expect(wrapper.find(".fz__table__header--radio").exists()).toBe(false);
        const headers = wrapper.findAll('[role="columnheader"]');
        expect(headers.length).toBe(2);
      });

      it("should render accordion variant with accordion header cell", async () => {
      ...
      it("should render list variant without column headers", async () => {
      ...
      it("should render radio variant with radio header cell", async () => {
        const wrapper = createWrapper({ variant: "radio" });
        await wrapper.vm.$nextTick();
        expect(wrapper.find(".fz__table__header--radio").exists()).toBe(true);
      });
    });
  ...
  describe("Events", () => {
    ...
    it("should emit fztable:newitem when new item button is clicked", async () => {
      ...
      const newItemBtn = buttons.find(
        (btn) => btn.text().includes("Nuovo") || btn.text().includes("New"),
      );
  ...
  describe("Accordion variant", () => {
    const accordionData = [
      ...
      },
      {
        ...
        subRows: [{ id: "sub-3", name: "Sub 3", email: "sub3@example.com" }],
      },
      {
        id: "row-gamma",
        name: "No SubRows",
    ...
    describe("leftColIcon reflects open state using row.id", () => {
      ...
      it("should apply text-blue-500 class to leftColIconClass when row with id is open", async () => {
        ...
        const rowsAfterClick = wrapper.findAllComponents(FzRow);
        const parentRowAfterClick = rowsAfterClick[0];
        expect(parentRowAfterClick.props("leftColIconClass")).toBe(
          "text-blue-500",
        );
      });
    });
    ...
    describe("subrow actions receive subrow data", () => {
      ...
      it("should resolve different actions for each subrow when using a function", async () => {
        ...
        if (
          typeof sub1Actions !== "function" &&
          typeof sub2Actions !== "function"
        ) {
          ...
          const r1 =
            typeof sub1Actions === "function"
              ? sub1Actions(accordionData[0].subRows[0])
              : sub1Actions;
          const r2 =
            typeof sub2Actions === "function"
              ? sub2Actions(accordionData[0].subRows[1])
              : sub2Actions;
    ...
    describe("openRowIds v-model", () => {
      it("should work in uncontrolled mode without openRowIds prop", async () => {
        const wrapper = createWrapper({
          modelValue: accordionData,
          variant: "accordion",
        });
        ...
        const rowsAfter = wrapper.findAllComponents(FzRow);
        expect(rowsAfter[0].props("leftColIcon")).toBe("angle-up");
      });

      it("should render subrows for initially open rows from v-model", async () => {
        const wrapper = createWrapper({
          modelValue: accordionData,
          variant: "accordion",
          openRowIds: new Set(["row-alpha"]),
        });
        await wrapper.vm.$nextTick();

        const rows = wrapper.findAllComponents(FzRow);
        // row-alpha (parent) + 2 subrows + row-beta (parent) + row-gamma (parent) = 5
        expect(rows.length).toBe(5);
        expect(rows[0].props("leftColIcon")).toBe("angle-up");
        expect(rows[0].props("leftColIconClass")).toBe("text-blue-500");
        // row-beta should remain closed
        expect(rows[3].props("leftColIcon")).toBe("angle-right");
      });

      it("should open rows programmatically via prop update", async () => {
        const wrapper = createWrapper({
          modelValue: accordionData,
          variant: "accordion",
          openRowIds: new Set<string | number>(),
        });
        await wrapper.vm.$nextTick();

        // All closed initially
        let rows = wrapper.findAllComponents(FzRow);
        expect(rows.length).toBe(3);

        await wrapper.setProps({ openRowIds: new Set(["row-beta"]) });
        await wrapper.vm.$nextTick();

        rows = wrapper.findAllComponents(FzRow);
        // row-alpha + row-beta + 1 subrow + row-gamma = 4
        expect(rows.length).toBe(4);
      });

      it("should close rows programmatically via prop update", async () => {
        const wrapper = createWrapper({
          modelValue: accordionData,
          variant: "accordion",
          openRowIds: new Set(["row-alpha", "row-beta"]),
        });
        await wrapper.vm.$nextTick();

        // Both open: 3 parents + 2 sub-alpha + 1 sub-beta = 6
        let rows = wrapper.findAllComponents(FzRow);
        ...
        rows = wrapper.findAllComponents(FzRow);
        // row-alpha (closed) + row-beta + 1 subrow + row-gamma = 4
        expect(rows.length).toBe(4);
      });

      it("should emit update:openRowIds when a row is clicked to open", async () => {
        const wrapper = createWrapper({
          modelValue: accordionData,
          variant: "accordion",
          openRowIds: new Set<string | number>(),
        ...
        const emitted = wrapper.emitted("update:openRowIds");
        expect(emitted).toBeTruthy();
        const lastEmitted = emitted![emitted!.length - 1][0] as Set<
          string | number
        >;
        expect(lastEmitted.has("row-alpha")).toBe(true);
      });

      it("should emit update:openRowIds when a row is clicked to close", async () => {
        const wrapper = createWrapper({
          modelValue: accordionData,
          variant: "accordion",
          openRowIds: new Set<string | number>(["row-alpha"]),
        });
        await wrapper.vm.$nextTick();
        ...
        const lastEmitted = emitted![emitted!.length - 1][0] as Set<
          string | number
        >;
        expect(lastEmitted.has("row-alpha")).toBe(false);
      });

      it("should handle openRowIds with id that has empty subRows without crashing", async () => {
        const wrapper = createWrapper({
          modelValue: accordionData,
          variant: "accordion",
          openRowIds: new Set(["row-gamma"]),
        });
        await wrapper.vm.$nextTick();

        // row-gamma has subRows: [], so no subrows rendered even if "open"
        const rows = wrapper.findAllComponents(FzRow);
        expect(rows.length).toBe(3);
      });

      it("should handle openRowIds with non-existent id without crashing", async () => {
        const wrapper = createWrapper({
          modelValue: accordionData,
          variant: "accordion",
          openRowIds: new Set(["nonexistent-id"]),
        });
        await wrapper.vm.$nextTick();
      ...
      it("should close all rows when openRowIds is set to empty Set", async () => {
        const wrapper = createWrapper({
          modelValue: accordionData,
          variant: "accordion",
          openRowIds: new Set(["row-alpha", "row-beta"]),
        ...
        await wrapper.setProps({ openRowIds: new Set() });
        await wrapper.vm.$nextTick();
