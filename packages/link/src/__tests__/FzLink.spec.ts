...
describe('FzLink', () => {
  ...
  // Visual-invariance guard for the self-contained fz-link.css.
  // The component keeps ALL its original Tailwind utility classes (so wherever
  // those utilities are generated, rendering is byte-identical). The shipped
  // CSS only adds parallel `fz-link*` classes that reference the SAME tokens.
  // These tests lock in that both layers coexist for every combination, so the
  // self-contained CSS can never diverge visually from the utility output.
  describe('Self-contained CSS (visual invariance)', () => {
    const mountLink = (props: Record<string, unknown>) =>
      mount(FzLink, {
        props: { to: '/example', ...props },
        slots: { default: 'Link' },
        global: { plugins: [router] }
      })

    it('default + no-underline: original utilities kept, fz-link modifiers added', () => {
      const link = mountLink({ type: 'default', linkStyle: 'default' }).find('a')
      // original utilities — unchanged
      expect(link.classes()).toEqual(
        expect.arrayContaining(['text-blue-500', 'hover:text-blue-600', 'hover:underline'])
      )
      // new parallel classes — same tokens
      expect(link.classes()).toEqual(
        expect.arrayContaining(['fz-link', 'fz-link--default', 'fz-link--no-underline'])
      )
      expect(link.classes()).not.toContain('fz-link--danger')
    })

    it('default + underline: original utilities kept, fz-link modifiers added', () => {
      const link = mountLink({ type: 'default', linkStyle: 'underline' }).find('a')
      expect(link.classes()).toEqual(
        expect.arrayContaining(['text-blue-500', 'hover:text-blue-600', 'underline'])
      )
      expect(link.classes()).toEqual(
        expect.arrayContaining(['fz-link', 'fz-link--default', 'fz-link--underline'])
      )
    })

    it('danger + no-underline: original utilities kept, fz-link modifiers added', () => {
      const link = mountLink({ type: 'danger', linkStyle: 'default' }).find('a')
      expect(link.classes()).toEqual(
        expect.arrayContaining([
          'text-semantic-error-200',
          'hover:text-semantic-error-300',
          'hover:underline'
        ])
      )
      expect(link.classes()).toEqual(
        expect.arrayContaining(['fz-link', 'fz-link--danger', 'fz-link--no-underline'])
      )
      expect(link.classes()).not.toContain('fz-link--default')
    })

    it('disabled default/danger (span): original utilities kept, fz-link--disabled added', () => {
      const def = mountLink({ type: 'default', disabled: true }).find('span')
      expect(def.classes()).toContain('text-blue-200')
      expect(def.classes()).toEqual(
        expect.arrayContaining(['fz-link', 'fz-link--default', 'fz-link--disabled'])
      )

      const danger = mountLink({ type: 'danger', disabled: true }).find('span')
      expect(danger.classes()).toContain('text-semantic-error-100')
      expect(danger.classes()).toEqual(
        expect.arrayContaining(['fz-link', 'fz-link--danger', 'fz-link--disabled'])
      )
    })
  })
})
