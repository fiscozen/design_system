...
- f598a9b: `FzNavbar`: new `elevation` prop (`'raised' | 'flat'`, default `'raised'`) to switch the elevation shadow off — for a navbar that sits _on_ the page background rather than above it, like the backoffice rail inside `FzFrameTemplate`, where a shadow draws a fake edge along a surface with nothing behind it.
