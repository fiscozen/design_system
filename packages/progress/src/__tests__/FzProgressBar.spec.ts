...
describe("FzProgressBar", () => {
  describe("Rendering", () => {
    ...
    it("renders progress fill with correct classes", () => {
      ...
      expect(fill.classes()).toContain("motion-safe:transition-all");
      expect(fill.classes()).toContain("motion-safe:duration-300");
    });
    ...
    it.each([
      ...
      "applies color-specific background to container when color is %s",
      (color, expectedClass) => {
        const wrapper = mount(FzProgressBar, {
  ...
  describe("Accessibility", () => {
    ...
    it("has default aria-label when label not provided", () => {
      ...
      const container = wrapper.find(".fz-progress-bar");
      expect(container.attributes("aria-label")).toBe("Avanzamento");
    });

    it("omits aria-valuetext when valueText prop is not provided", () => {
      const wrapper = mount(FzProgressBar, {
        props: {
          current: 50,
        },
    ...
    it("renders aria-valuetext when valueText prop is provided", () => {
      ...
      const container = wrapper.find(".fz-progress-bar");
      expect(container.attributes("aria-valuetext")).toBe(
        "Caricamento file 3 di 10",
      );
    });
    ...
    it("has all ARIA attributes for complete accessibility", () => {
      const wrapper = mount(FzProgressBar, {
        props: {
          ...
          valueText: "Scaricamento 30 di 50",
        },
      });
